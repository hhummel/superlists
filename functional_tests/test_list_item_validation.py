...
class ItemValidationTest(FunctionalTest):
    ...
    def test_cannot_add_empty_list_items(self):
        ...
        self.browser.find_element_by_id('id_new-item').send_keys('Buy milk') 
        self.browser.find_element_by_id('id_new-item').send_keys(Keys.ENTER)
        self.wait_for_row_in_list_table('1: Buy milk')

        #Perversely, she now decideds to submit a second blank list item
        self.browser.find_element_by_id('id_new-item').send_keys(Keys.ENTER)
        
        #She receives a similar warning on the list page
        self.wait_for(lambda: self.assertEqual(self.browser.find_element_by_css_selector('.has-error').text, "You can't have an empty list item"))

        #And she can correct it buy filling some text in
        self.browser.find_element_by_id('id_new-item').send_keys('Make tea') 
        self.browser.find_element_by_id('id_new-item').send_keys(Keys.ENTER)
        self.wait_for_row_in_list_table('1: Buy milk')
